...
**Audit (agent glance):** Navbar `fixed` + `Z_NAVBAR`. **Insights v2:** SSR `meta.scopeLabel` (admin doctor drill-down); scope+period chart subtitles; chart empty overlays; period-scoped data + invalidation unchanged; KPI/badges all-time; **328** tests + tsc + lint + build pass. Redis insights cache (`insights-redis-cache.ts`, 90s TTL). **Sonner Phase 3b (done).**

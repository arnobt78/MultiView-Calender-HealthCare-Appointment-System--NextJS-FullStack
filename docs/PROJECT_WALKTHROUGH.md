...
- **Panels:** [Today | Upcoming] (`lg:2-col`) · [Weekly Hours \| Global visit types] (`lg:2-col`) · [Unavailable Dates \| Additional Appointment Types] (`lg:2-col`) · **Related Billing** (`DoctorPortalInvoicesCard`) · **Related Patients** (`DoctorPortalPatientsCard`). Stacked headers: possessive title, `PortalPanelCountBadge`, inline colored status counts (`InvoiceStatusCountInlineRow`), muted subtitle. Glass tints: `doctorPortalBillingPanelClass` (sky), `doctorPortalPatientsPanelClass` (emerald). Collapsible add chips on schedule/type panels. Shared editors under `src/components/shared/doctor-settings/`.
...
- **Related Billing:** `DoctorPortalInvoicesCard` — `InvoiceStatusCountInlineRow` (Draft/Sent/Paid/… text colors); rows: `Invoice #id`, `InvoiceVisitSummaryLine` (calendar/clock/location), type+duration badge, patient portrait+care tier, `InvoicePortalListMetaRow`. Invoice delete: `ConfirmActionDialog` via `InvoiceAdminActionsMenu`. SSR: `prefetchInvoices` + visit summaries on `doctor-portal/page.tsx`.

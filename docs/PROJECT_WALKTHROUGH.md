...
- **Insights v2:** `/insights` — Recharts-only `AnalyticsPage`; **scope** in chrome (`InsightsScopeToolbar`, inline — no outer shell); **period** in Appointments (`InsightsAppointmentsPanelHeader` + `InsightsGlassSegment`, dashboard-style tab pills); KPI strip = calendar windows (not View-as); Appointments badges (Overall / Until today / Upcoming) = all-time in scope; **period-scoped** — charts (trend, weekday, status-over-time, category, visit types, age, top patients, paid revenue, invoice status), **By status** (`countAppointmentsByStatusInRange` + sky `periodLabel`), **avg duration** (`fetchAvgDurationMinutesInRange`), **patients.newInPeriod** (`countDistinctPatientsInPeriodToNow`, start→now), **org doctor/specialty** (`fetchDoctorBreakdown(period)`); **`v2.meta.periodLabel`** via `insights-period-label.ts` (week bracketed from–to dates); chart/By-status subtitles via `insights-chart-subtitle.ts`; static copy in `detailHint` tooltips; **`AnalyticsChartCard`** single `CardContent`; TanStack `queryKeys.insights.filter({scope, doctorId?, period})` + `invalidateInsightsAndAnalytics`; Redis read-through (`insights-redis-cache.ts`, 90s); SSR `prefetchInsights` + navbar `prefetchInsightsNav`; API `?scope&doctorId&period`; buster **`v5`**; tests **304** incl. `insights-period-label`, `insights-period-charts`.
...
**Audit (agent glance):** Navbar `fixed` + `Z_NAVBAR`. **Insights v2 + filter wiring:** scope/period query + URL sync; period-scoped charts + By status + avg duration + `newInPeriod`; bracketed `periodLabel` (week from–to); sky subtitles on charts + By status; KPI/badges intentionally all-time; `insights.root` invalidation on CRUD; SSR `prefetchInsights` + `useInsights`; **304** tests + tsc + lint + build pass. Redis insights cache (`insights-redis-cache.ts`, 90s TTL). **Sonner Phase 3b (done).**

...
- **Insights v2:** `/insights` — Recharts-only `AnalyticsPage`; **scope** in chrome (`InsightsScopeToolbar`, inline — no outer shell); **period** in Appointments (`InsightsAppointmentsPanelHeader` + `InsightsGlassSegment`, dashboard-style tab pills); KPI strip = calendar windows (not View-as); Appointments badges (Overall / Until today / Upcoming) = all-time in scope; **period-scoped** charts + **By status** + avg duration + `newInPeriod` + org doctor/specialty; chart subtitles = **scope + period** (`insights-scope-display.ts` + `insights-chart-subtitle.ts` → `getInsightsChartContextSubtitle`); **empty charts** — placeholder axes + `AnalyticsChartEmptyOverlay` per card (`analytics-chart-empty.ts`); Top patients table empty row; **`AnalyticsChartCard`** single `CardContent`; TanStack `queryKeys.insights.filter({scope, doctorId?, period})` + `invalidateInsightsAndAnalytics`; Redis read-through (`insights-redis-cache.ts`, 90s); SSR `prefetchInsights` + navbar `prefetchInsightsNav`; API `?scope&doctorId&period`; buster **`v5`**; tests **320** incl. `analytics-chart-empty`, `insights-scope-display`.
...
**Audit (agent glance):** Navbar `fixed` + `Z_NAVBAR`. **Insights v2:** scope+period chart subtitles; chart empty overlays with axis placeholders; period-scoped data + invalidation unchanged; KPI/badges all-time; **320** tests + tsc + lint + build pass. Redis insights cache (`insights-redis-cache.ts`, 90s TTL). **Sonner Phase 3b (done).**

...
- **Verify (pre-commit):** `npm test && npx tsc --noEmit && npm run lint && npm run build` — **352 tests** (52 files); includes `analytics-chart-interaction.test.ts`, `query-cache-cross-tab.test.ts`, insights `period=all`, `insights-all-time-prisma`, `query-client-insights-invalidation`.

...
- **Insights v2:** `/insights` — Recharts-only `AnalyticsPage`; **scope** in chrome (`InsightsScopeToolbar`), **period** in Appointments section (`InsightsPeriodControls`); KPI totals include scheduled future (full calendar windows for YTD/month/week); badge + status aligned; **period-aware trend** (`month` = daily in calendar month, `year` = Jan–Dec, shared with Paid Revenue); **period-aware status + weekday** charts; **chart UX** (`AnalyticsChartTooltip` z-index + value labels, pie slice config); **portal-style chart headers** (`AnalyticsChartPanelHeader`, icon on every chart); `fetchStatusOverTimeCalendarYear`; busiest weekday + status over time follow chart period; TanStack `insights.root` invalidation + Redis read-through (`insights-redis-cache.ts`, 90s); assignee + back-nav bust; API `?scope&doctorId&period`; buster **`v5`**; tests **295** incl. `insights-redis-cache`, `insights-trend-buckets`, `insights-period-charts`, `analytics-chart-interaction`, `query-client-insights-invalidation`.
...
**Audit (agent glance):** Navbar `fixed` + `Z_NAVBAR`. **Insights v2 + polish:** scope/period split toolbar, future-inclusive KPIs, period-aware trend/revenue/status/weekday charts, chart tooltips/labels, portal-style chart headers (required icons), live TanStack `insights.root` invalidation (assignee/sharing/back-nav), SSR `prefetchInsights` + `useInsights` keyed by `{scope, doctorId?, period}`; aggregate-only chart path; **295** tests + build pass; SQL weekday DOW, period-scoped invoice status, legacy `monthlyData` from trend. Redis insights cache (`insights-redis-cache.ts`, 90s TTL, bust on `invalidateDashboardOverview`). **Sonner Phase 3b (done).**

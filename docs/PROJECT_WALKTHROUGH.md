...
- **Verify (pre-commit):** `npm test && npx tsc --noEmit && npm run lint && npm run build` — **621 tests** (112 files); includes `doctor-portal-billing-display.test.ts`, `doctor-portal-patients-display.test.ts`, `invoice-list-row-display.test.ts`, `crud-notify-messages.test.ts`.

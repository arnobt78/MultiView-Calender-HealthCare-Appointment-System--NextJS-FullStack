...
- **Insights v2:** `/insights` — Recharts-only `AnalyticsPage`; **scope** in chrome (`InsightsScopeToolbar`, inline — no outer shell); **period** in Appointments (`InsightsAppointmentsPanelHeader` + `InsightsGlassSegment`, dashboard-style tab pills); KPI strip = calendar windows (not View-as); Appointments badges (Overall / Until today / Upcoming) = all-time in scope; **period-scoped** charts + **By status** + avg duration + `newInPeriod` + org doctor/specialty; chart subtitles = **scope + period** (`resolveInsightsScopeLabelForMeta` in `getInsightsData` for SSR/API/Redis — doctor name on admin drill-down; client fallback via `resolveInsightsScopeChartLabel`); `getInsightsChartContextSubtitle`; **empty charts** — placeholder axes + `AnalyticsChartEmptyOverlay` (`analytics-chart-empty.ts`); `AnalyticsTopPatientsPanel` (no headers when empty, centered body); empty copy stacked rows (`AnalyticsChartEmptyStateCopy`); **`AnalyticsChartCard`** single `CardContent`; TanStack `queryKeys.insights.filter({scope, doctorId?, period})` + `invalidateInsightsAndAnalytics`; Redis read-through (`insights-redis-cache.ts`, 90s); SSR `prefetchInsights` + navbar `prefetchInsightsNav`; tests **328** incl. `insights-scope-label-meta`.; API `?scope&doctorId&period`; buster **`v5`**; tests **320** incl. `analytics-chart-empty`, `insights-scope-display`.
...
**Audit (agent glance):** Navbar `fixed` + `Z_NAVBAR`. **Insights v2:** SSR `meta.scopeLabel`; inline chart title+subtitle row; chart empty overlays + top patients empty panel; period-scoped data + invalidation unchanged; KPI/badges all-time; **328** tests + tsc + lint + build pass. Redis insights cache (`insights-redis-cache.ts`, 90s TTL). **Sonner Phase 3b (done).**

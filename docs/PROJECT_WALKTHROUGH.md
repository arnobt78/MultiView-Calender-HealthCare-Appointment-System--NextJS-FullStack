...
- **Verify (pre-commit):** `npm test && npx tsc --noEmit && npm run lint && npm run build` — **623 tests** (113 files); includes `doctor-portal-billing-display.test.ts`, `doctor-portal-patients-display.test.ts`, `confirm-delete-dialog-copy.test.tsx`, `invoice-list-row-display.test.ts`, `crud-notify-messages.test.ts`.

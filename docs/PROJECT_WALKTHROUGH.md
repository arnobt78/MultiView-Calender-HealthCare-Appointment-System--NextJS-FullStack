...
- **Related Billing:** `DoctorPortalInvoicesCard` — `InvoiceStatusCountInlineRow`; rows: sky `doctorPortalInvoiceHeaderStripClass`, title+duration inline, owner `DoctorIdentityCell`, `InvoicePortalListMetaRow` (Due/Created/Paid|Refunded + linked issuer via `EntityDetailAuditActorInline`). SSR: `prefetchInvoices` + visit summaries.

...
/** Read-only summary row — mirrors selected type tile (doctor picker collapse pattern). */
function PatientBookingTypeSummaryCard({
  type,
  flexLabel,
}: {
...
/**
 * Appointment type list — collapses to selected summary + “Change appointment type”
 * (same UX as `DoctorDirectoryPickerList`).
 */
export function PatientBookingTypePickerList({
  typesLoading,
  isFlexible,
  types,
  selectedType,
  ...
  fillLayout = false,
  className,
}: PatientBookingTypePickerListProps) {
    ...
    if (!showList && flexCollapsed) {
      return (
        <div className={cn("space-y-2 px-2 py-1", className)}>
          <PatientBookingTypeSummaryCard flexLabel={`Flexible booking · ${flexDuration} min`} />
          <Button
            type="button"
            variant="ghost"
            size="sm"
  ...
  if (typeCollapsed && selectedType) {
    return (
      <div className={cn("space-y-2 px-2 py-1", className)}>
        <PatientBookingTypeSummaryCard type={selectedType} />
        <Button
          type="button"
          variant="ghost"
          size="sm"
